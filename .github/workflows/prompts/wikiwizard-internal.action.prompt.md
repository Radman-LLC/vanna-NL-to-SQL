...
**Analyze only code that was added or modified in this branch** (use `git diff origin/develop...HEAD` with THREE dots to exclude merged commits)
...
**Scope:**
- Focus only on code that was added or modified in this branch using `git diff origin/develop...HEAD` (THREE dots to exclude merged commits)
...
- Use the repository's `copilot-instructions.md` for guidance on style and conventions
...
**Step 1: Run Git Diff**
Run `git diff origin/develop...HEAD` (THREE dots) to get ONLY changes from this branch, excluding merged commits. Focus on code files: .cs, .js, .ts, .tsx, .py, .csproj, .sln, Dockerfile, .config, etc.
...
- [ ] Run `git diff origin/develop...HEAD` (THREE dots) to see ONLY this branch's changes
